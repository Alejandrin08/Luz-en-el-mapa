...
import { stories, getCap4Key } from "../data/storyContent.js";
import { useEffect } from "react";

export default function HistoriaCuatro() {
  ...
  const decisionCap3 = decisiones.cap3 || "a";
  const nombre = config.nombre;
  const cap4Key = getCap4Key(decisionCap1, decisionCap2, decisionCap3);
  const contexto = config.contexto || "escolar";
  const edad = config.rango || "infancia";
  const data = stories[contexto][edad]?.capitulo4[cap4Key];
  const capKeyPrefix = `${contexto}_cap4_${cap4Key}`;
    ...
    <SiteLayout>
      <div className="mx-auto max-w-3xl px-6 py-10">
        ...
        <article className="mt-8 rounded-xl border bg-card p-6 md:p-8">
          ...
          <div className="mt-4 space-y-4 text-foreground/90 leading-relaxed">
            {data.descripcion.map((bloque, i) => {
              ...
              if (bloque?.tipo === "dato") {
                const idx = datoIdx++;
                const clave = `cap4_${cap4Key}_${idx}`;
                return (
                  <DatoDinamico
                    key={i}
        ...
        <div className="mt-10 border-t pt-6 flex items-center justify-between">
          <Link to="/historiaTres" className="inline-flex items-center gap-2 rounded-md border border-input bg-card px-4 py-2.5 text-sm font-medium hover:bg-accent">
